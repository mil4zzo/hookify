                ...
                with st.container(border=True):
                    ...
                    if "time_range" in st.session_state:
                        ...
                        with cols_time_range[1]:
                            st.markdown(pd.to_datetime(time_range["since"]).strftime("%d/%m/%Y") + " to " + pd.to_datetime(time_range["until"]).strftime("%d/%m/%Y"))
                    ...
                    if "filters" in st.session_state:
                        ...
                        with cols_filters[0]:
                            st.caption("Filters:")
                        with cols_filters[1]:
                            st.markdown(" | ".join(f'{str(filter["field"].split(".")[0]).capitalize()} *:gray[{str(filter["operator"]).lower()}]* **{filter["value"]}**' for filter in filters))
