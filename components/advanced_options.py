...
class AdvancedOptions:
    def __init__(self):
        ...
        if 'min_plays' not in st.session_state:
            st.session_state.min_plays = 25
        if 'min_spend' not in st.session_state:
            st.session_state.min_spend = 50
                ...
                with st.container(border=True):
                    ...
                    with filters.container():
                        ...
                        with cols[0]:
                            st.write('Campaign')
                        with cols[1]:
                            st.multiselect('Select campaign:', campaign_list, 
                                                        label_visibility='collapsed',
                        ...
                        with cols[0]:
                            st.write('Adset')
                        with cols[1]:
                            st.multiselect('Select adset:', adset_list, 
                                                    label_visibility='collapsed',
                                                    key='filters_adset')
                        # FILTERS > ADs
                        cols = st.columns([1,6], gap='small')
                        with cols[0]:
                            st.write('Ad name')
                        with cols[1]:
                            st.multiselect('Select ad:', ad_list, 
                                                label_visibility='collapsed',
                                                key='filters_adname')
                    ...
                    with thresholds.container():
                        ...
                        with cols[1]:
                            st.number_input("Minimum Plays", 
                                                        ...
                                                        value=st.session_state.min_plays, 
                                                        step=5, 
                                                        label_visibility='collapsed',
                                                        key='min_plays')
                        cols = st.columns([1,2], gap='small')
                        ...
                        with cols[1]:
                            st.number_input("Minimum Spend", 
                                                        ...
                                                        value=st.session_state.min_spend, 
                                                        step=10, 
                                                        label_visibility='collapsed',
                                                        key='min_spend')
                            
                st.form_submit_button('Apply filters', type='primary', use_container_width=True, on_click=self.set_apply_filters, kwargs={'cost_column': cost_column})

    def set_apply_filters(self, cost_column):
        st.session_state.apply_filters = True
        # st.session_state.filters_campaign = 
        # st.session_state.filters_adset = 
        # st.session_state.filters_adname = 
        # st.session_state.min_plays = 
        # st.session_state.min_spend = 
