    ...
    <PageContainer variant="analytics" title="G.O.L.D." description="Classificação de anúncios por performance">
      ...
      {actionType && serverAverages && (
        <AnalyticsWorkspace className="gap-8 overflow-visible">
          <GoldKanbanWidget ads={validatedRankings as RankingsItem[]} averages={serverAverages} actionType={actionType} validationCriteria={validationCriteria} names={names} dateStart={dateRange.start} dateStop={dateRange.end} availableConversionTypes={actionTypeOptions} packIds={Array.from(selectedPackIds)} />
