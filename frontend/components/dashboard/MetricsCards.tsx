...
import { AggregatedData, formatPercentage, formatNumber } from "@/lib/utils/aggregation";
import { useFormatCurrency } from "@/lib/utils/currency";
import { IconTarget, IconCurrencyDollar, IconEye, IconPointer, IconUsers, IconTrendingUp, IconPlayerPlay, IconExternalLink, IconUser } from "@tabler/icons-react";
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="text-center">
            <div className="text-3xl font-bold text-brand">{formatPercentage(data.retention_at_3 * 100, 0)}</div>
            <div className="text-sm text-muted-foreground">Retention at 3s</div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="text-center">
              ...
              <div className="text-xs text-muted-foreground">Thruplays</div>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="flex items-center gap-2 text-lg">
            <IconCurrencyDollar className="w-5 h-5 text-yellow-500" />
            💵 Budget
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="flex items-center gap-2 text-lg">
            <IconPointer className="w-5 h-5 text-green-500" />
            👆 Clicks
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="text-center">
              <div className="text-lg font-semibold text-green-500">{formatPercentage(data.ctr * 100, 2)}</div>
            ...
            <CardContent className="p-3">
              <div className="flex items-center justify-between">
                <div className="text-center flex-1">
                  <div className="text-xl font-bold text-green-500">{formatPercentage(data.website_ctr * 100, 2)}</div>
                  <div className="text-xs text-muted-foreground">Website CTR</div>
                </div>
                <div className="text-right">
                  <div className="text-sm text-muted-foreground">{formatNumber(data.inline_link_clicks)} clicks</div>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-border">
            <CardContent className="p-3">
              <div className="flex items-center justify-between">
                <div className="text-center flex-1">
                  <div className="text-xl font-bold text-green-500">{formatPercentage(data.profile_ctr * 100, 2)}</div>
                  <div className="text-xs text-muted-foreground">Profile CTR</div>
                </div>
                <div className="text-right">
                  <div className="text-sm text-muted-foreground">{formatNumber(data.clicks - data.inline_link_clicks)} clicks</div>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-semibold">{formatPercentage(data.connect_rate * 100, 0)}</span>
            </div>
            <Progress value={data.connect_rate * 100} className="h-2" />
          </div>
        ...
        <CardContent>
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="text-center">
              ...
              <div className="text-xs text-muted-foreground">Adsets</div>
            </div>
            <div className="text-center">
