...
import { IconPlayerPlay, IconCheck, IconAlertTriangle, IconAlertCircle, IconChevronLeft, IconChevronRight, IconSparkles } from "@tabler/icons-react";
...
const ROW_BASE_CLASS = "grid grid-cols-7 gap-2 py-2 px-3 rounded items-center";
...
// Função para determinar o status da métrica e retornar o ícone correspondente
function getMetricStatusIcon(current: number, average: number, lowerIsBetter: boolean = false) {
  ...
  if (lowerIsBetter) {
    ...
    if (current <= average) {
      // Verde: check
      return <IconCheck className="w-full h-full text-green-600 dark:text-green-400" />;
    }

    const ratio = current / average;
    if (ratio > 1 && ratio <= 1.25) {
      // Amarelo: warning
      return <IconAlertTriangle className="w-full h-full text-yellow-600 dark:text-yellow-400" />;
    }
    if (ratio > 1.25 && ratio <= 1.5) {
      // Laranja: siren
      return <IconAlertCircle className="w-full h-full text-orange-600 dark:text-orange-400" />;
    }
    if (ratio > 1.5) {
      // Vermelho: siren
      return <IconAlertCircle className="w-full h-full text-red-600 dark:text-red-400" />;
    }
  } else {
    // Para métricas onde maior é melhor (ex: Hook, CTR, etc)
    if (current >= average) {
...
function OpportunityCard({ row, idx, formatCurrency, avgHook, avgHoldRate, avgWebsiteCtr, avgConnectRate, avgPageConv, avgCpr, metricRanks, onAdClick, onInsightsClick, isInOverlay = false }: { row: OpportunityRow; idx: number; formatCurrency: (value: number) => string; avgHook: number; avgHoldRate: number; avgWebsiteCtr: number; avgConnectRate: number; avgPageConv: number; avgCpr: number; metricRanks: MetricRanks; onAdClick?: (row: OpportunityRow, openVideo?: boolean) => void; onInsightsClick?: () => void; isInOverlay?: boolean }) {
  ...
  return (
    <div
      className={`bg-card border border-border rounded-lg overflow-hidden flex flex-col flex-shrink-0 transition-shadow ${isInOverlay ? "" : "cursor-pointer hover:shadow-lg"}`}
      style={{
        aspectRatio: "9/16",
        width: "280px",
      }}
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 flex items-center justify-center px-4 relative">
          <div className="w-full h-full rounded-lg bg-muted flex items-center justify-center relative overflow-hidden">
            ...
            {/* Botão de play */}
            <button
              className="relative z-10 w-16 h-16 rounded-full bg-primary hover:bg-primary-90 transition-colors flex items-center justify-center shadow-lg"
              onClick={(e) => {
                e.stopPropagation();
                if (onAdClick) {
                  onAdClick(row, true);
        ...
        <div className="p-4 flex-shrink-0 space-y-2">
          ...
          {/* Hook */}
          <div className={ROW_MUTED_CLASS} style={hookBadgeStyles || undefined}>
            <div className={`col-span-3 ${hookBadgeStyles ? "" : "text-foreground"} font-semibold text-xs flex items-center gap-2 min-w-0`} style={hookBadgeStyles ? { color: hookBadgeStyles.color } : undefined}>
              <div className="flex-shrink-0 w-4 h-4 flex items-center justify-center">{hookEmoji ? <span className="text-base leading-none">{hookEmoji}</span> : getMetricStatusIcon(r.hook, avgHook)}</div>
...
function MetricRow({ label, current, average, formatValue, medalEmoji, badgeStyles }: { label: string; current: number; average: number; formatValue: (v: number) => string; medalEmoji?: string; badgeStyles?: CSSProperties | null }) {
  return (
    <div className={ROW_MUTED_CLASS} style={badgeStyles || undefined}>
      <div className={`col-span-3 ${badgeStyles ? "" : "text-foreground"} font-semibold text-xs flex items-center gap-2 min-w-0`} style={badgeStyles ? { color: badgeStyles.color } : undefined}>
        <div className="flex-shrink-0 w-4 h-4 flex items-center justify-center">{medalEmoji ? <span className="text-base leading-none">{medalEmoji}</span> : getMetricStatusIcon(current, average, false)}</div>
