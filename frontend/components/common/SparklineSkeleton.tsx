  ...
  return (
    <div className={`flex items-end justify-between ${sizeClass} ${className}`.trim()} style={{ gap: "2px" }}>
      {displayHeights.map((height, index) => (
        <div
          key={index}
          className={`${staggeredFadeOut ? "sparkline-exit-bar" : "sparkline-wave-bar"} flex-1 bg-gradient-to-b from-muted-50 to-muted-20 border-t border-muted-foreground-40`}
          style={{
            height: `${height}%`,
