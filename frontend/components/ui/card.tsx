...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => <div ref={ref} data-slot="card" className={cn("bg-card text-card-foreground flex flex-col gap-4 rounded-md border border-border p-6 shadow-elevation-raised @container/card", className)} {...props} />);
