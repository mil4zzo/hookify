...
- **Pack fechado não anda (2026-09-17)**: com "manter atualizado" **desligado**, o período do pack é decisão do usuário. A atualização busca no máximo até a data final do pack — tanto "dados mais recentes" quanto "todo o período". Antes, qualquer atualização reabria o pack até hoje em silêncio. Com o toggle **ligado**, o fim acompanha o presente. A regra vive em `backend/app/services/pack_window.py` e é espelhada em `frontend/lib/utils/refreshWindow.ts` (o diálogo mostra a data que vai ser pedida de fato).
- **Só uma atualização por pack (2026-09-17, migration 166)**: marcar o pack como "atualizando" é um compare-and-set no banco (`pack_acquire_refresh_lock`): se o pack já está atualizando com prazo vigente — por outro membro, outra aba ou uma edição de período — a segunda tentativa recebe 409 e, quando há um job ativo, o app se reanexa a ele em vez de abrir um segundo relatório na Meta. A fila que o usuário vê ("um pack espera o outro") é do navegador e só serializa o que ele mesmo dispara; a trava do banco é o que vale entre pessoas. Prazo de 15 min; job que morre sem escrever o status final libera sozinho (varredura da 141).
