...
| F5.4 | `pg_dump` do schema + `generate_schema_map.py`; commit "migration 140 aplicada em produção" | `[~]` schema sincronizado (`schema.sql` + `schema_map.md`); commit pendente |
| F5.5 | Deploy no VPS (`git pull` + `deploy/deploy.sh`) | `[ ]` |
