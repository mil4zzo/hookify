...
**INCIDENTE (2026-09-07, manhã seguinte ao deploy) — R$ 12 mil zerados no `EI.30 - CA4 Cap`, e mais 5 packs.** O usuário comparou o pack com o Gerenciador (mesmos filtros, CSV exportado): Hookify R$ 520.187,74 × Gerenciador R$ 532.191,11. Chave a chave contra a consulta única do período (que bate com o Gerenciador ao centavo): **1.138 pares (anúncio, dia) com gasto real sobrescritos com zero** (R$ 11.780,89), concentrados no fim de julho; +R$ 218 de reafirmações de centavos; +R$ 3,64 de cauda sem gasto. Três peças, cada uma razoável sozinha:

1. **`attribution_setting` nos fields do relatório de anúncios** (meu, deployado na véspera) faz a Meta devolver linha para (quase) todo par (anúncio, dia) do período, com ou sem entrega — o relatório incha 3–4×. Evidência: em **2.380 jobs anteriores o máximo tinha sido 33 páginas / 16.309 linhas — neste mesmo pack, em 57 dias**; no primeiro dia com o campo, **6 packs bateram no teto de 100 páginas** (EI.30 CA4/CA5/CA6/CA7 Cap, EI.29 - Captação e EI.29 - Captação CA5). Ordem de grandeza no CA4 (BM1): 9 dias sem o campo = 6.585 linhas; 8 dias com = 27.317.
2. **`InsightsCollector` parava no teto (`MAX_PAGES=100` × 500 = 50.000 linhas) e devolvia `success=True`** com o recorte — só um `warning` no log.
3. **A síntese de linhas-zero do inventário** (`ad_inventory`, existe para o anúncio pausado aparecer com "entregou 0") tratou os anúncios das páginas não lidas como ausentes e gravou zero por cima do gasto verdadeiro. Packs pequenos batiam porque cabem em 50 mil linhas mesmo inchados — daí a pista do usuário ("pack menor da mesma época bate").

**Correção:** (a) o campo sai do relatório de anúncios; a janela vem de `GraphAPI.fetch_attribution_window`, consulta separada `level=adset` sem série diária (~1 linha por conjunto; medido: 151 conjuntos, <2 s), fail-open; (b) teto atingido com `next` sobrando → `success=False, complete=False, data=[]` e erro legível — **recorte nunca mais vira sucesso**; (c) `collection_is_complete()` é a única porta para a síntese de linhas-zero (trava explícita, além do `return`); (d) refresh que morre na coleta libera o pack (`failed`) em vez de deixá-lo `running` até o lock. Testes: `test_insights_collector_cap.py` (com sabotagem do teto) e `test_fetch_attribution_window.py` (trava que os dois clientes NÃO pedem o campo).

**Reparo:** o dado está intacto na Meta. Um "Todo o período" por pack afetado, depois do deploy, restaura os pares zerados (o relatório completo os traz com o gasto real e o upsert corrige por cima).
...
## ad_metrics com o pack na chave (migration 145) — 2026-09-07

**Problema.** Uma linha de `ad_metrics` era o anúncio-dia, compartilhada por todo pack que o contivesse (`ad_metric_pack_map` dizia quem). Tudo que se escreve NA LINHA vazava entre packs: a planilha do pack A gravava leadscore que o pack B lia; limpar A apagava B; atualizar A deixava B com metade dos dias mais maduros que a outra metade (a janela de atribuição amadurece conversões por dias — o pack B passava a comparar anúncios medidos com réguas diferentes). `delete_pack` classificava "linhas exclusivas" à mão e, quando o `try/except` falhava, deixava órfãs: 12.598 em produção, todas fora do período de qualquer pack vivo.

**Decisão.** Cada linha pertence a UM pack: PK `(user_id, pack_id, ad_id, date)` em `ad_metrics` e no read model `ad_performance_daily`. O mesmo anúncio-dia em dois packs são duas linhas. Produção não tinha nenhum (medido: 0 pares de packs do mesmo dono com anúncio-dia em comum), e a seleção simultânea de packs sobrepostos passou a ser bloqueada — `detect_pack_conflicts` perdeu a exceção "mesmo dono nunca conflita", que existia porque os dois liam a mesma linha física.
...
**Por que NÃO há FK de `ad_metrics` para `packs`.** Deletar um pack cascatearia ~77 mil linhas × 3 tabelas numa única instrução, sob o `statement_timeout` do PostgREST (e o do `service_role` é menor). `delete_pack` apaga por (pack, dia) em lotes pelo índice `(user_id, pack_id, date)` — ~40 requisições, cada uma bem abaixo do teto — e a FK do mapa e do read model leva o resto.

**Cirurgias que o porte revelou.** (1) A migration é `ALTER TABLE ADD COLUMN`, não tabela nova: `derive_row` recebe o tipo de linha de `ad_metrics` e lê campos por nome, então continua funcionando sem tocar. (2) O gatilho de UPDATE casava linha velha com nova por `(id, user_id)` — com o mesmo anúncio-dia em dois packs cruzaria 2×2 e recomporia o pack errado; passou a casar pela PK de 4 colunas. (3) O caminho "sem seleção" (`p_pack_ids` nulo = o silo inteiro) mantém o `GROUP BY` por (anúncio, dia): o bloqueio governa seleções, não o tudo. (4) O representante do grupo é codificado como `(impressões, ad_id, user_id, date)` e buscado por essas chaves; sem o pack na codificação, o join fecha com `pack = any(seleção)` — exato sem sobreposição, que é o invariante do bloqueio. (5) `packs_by_ad` (134) deixou de visitar o mapa: o pack está na linha. (6) 17 versões mortas de RPC foram apagadas na mesma migration — contra a chave nova cada uma devolveria número errado em silêncio se alguém a chamasse.
