...
>
> **O P3 está COMPLETO no código** — P3.1 a P3.8 (a P3.8 permanece pós-MVP por
> decisão). O compartilhamento funciona fim-a-fim: dono convida, convidado
> vê/analisa/atualiza/pausa/transcreve com a credencial do DONO, e desde a P3.5
> toda ação fica registrada com o ATOR.
>
> **Bloqueio único: nada disso está em produção.** O `main` está ~22 commits à
> frente do que roda no VPS (confirmado em 2026-08-22 pelo OpenAPI de produção:
> `UpdateStatusRequest` ainda sem `pack_ids`, containers `Up 3 days`). O deploy
> precisa ser rodado à mão no VPS (`git pull` + `deploy/deploy.sh`) — o MCP da
> Hostinger não dá shell e o `updateProject` só puxa imagens de registry, que não
> é como o Hookify constrói.
>
> **Sequência a partir daqui:** deploy → smoke em produção (dono no Manager +
> pack compartilhado como `test@hookify.com`) → migration `111` (DROP das colunas
> de julgamento por usuário, irreversível, só depois de estabilizar).
>
...
| P3.3b-resto | `packs/status-sync`, `transcribe` (dono\|editor — custa AssemblyAI), writes de entidade (status/budget/batch — exigem resolução entidade→silo+grant), re-attach/cancel de job p/ convidado | `Concluído` — 2026-08-21. **Feito:** writes de entidade (status individual/bulk, budget) e `packs/status-sync` multi-dono. O elo entidade→silo é `resolve_entity_write_scope`: o **contexto de pack vem do cliente e NÃO é escopo** — só propõe candidatos; autorizam (a) papel `dono|editor` no pack e (b) a entidade existir NAQUELE pack DENTRO do silo do dono. Pack forjado morre em (a), ad alheio em (b); dois silos estrangeiros → 409; silo do próprio ator sempre vence. **`Depends(get_graph_api)` teve de sair das rotas**: exige conexão Meta DO ATOR e levanta 403 antes do corpo — barrava justamente o convidado (mesma armadilha que a P3.3a achou no polling); o GraphAPI passa a ser montado no handler com a credencial do DONO. Escrita local pela convenção `user_jwt=None ⇒ service role` (`_sb_for`, 13 call sites). Token do dono expirado em ação de convidado: loga, não marca. Frontend threada o contexto até StatusCell/BudgetCell/bulk, **estabilizado por chave** (o default `[]` cria identidade nova por render) e com a dep que faltava no memo das colunas. **Mídia do convidado (2026-08-21):** o convidado via métricas mas não reproduzia vídeo/imagem — `/analytics/rankings/ad-id/{id}/creative` (silo do ator → sem `video_id`), `/facebook/video-source`, `/image-source` e `/ads/{id}/creative` (silo **e** token do ator, sem permissão na conta do dono). Convertidas; `resolve_entity_write_scope` virou `resolve_entity_pack_scope` com `roles` (leitura de mídia inclui `viewer`). Explicitado: **mesmo o convidado que TEM Facebook não pode usar o token dele** num pack compartilhado — a Meta negaria na conta do dono (o caso legítimo é a P3.8). **Transcrição (2026-08-21):** convertida. O bloqueio estimado em "~8 funções" eram 5 de escrita + 3 de leitura, **todas já filtrando `user_id` explicitamente** — ganharam `_sb_or_service(user_jwt)`, helper **local** e deliberadamente separado do `_get_sb` (aquele levanta quando falta auth; mudar o default dele para service role seria fail-open silencioso no repo inteiro). Gate `dono|editor` (custa AssemblyAI; viewer não gasta o saldo do dono), mas **ler** o estado é leitura e viewer passa. Leituras convertidas junto (`transcription-status`, `transcription-progress`, `/analytics/transcriptions/batch`, export CSV) — senão o convidado dispararia sem ver progresso, o "autoriza e falha no meio" que a P3.6 proíbe. No progresso o silo sai do **JOB**, nunca do ator. **Re-attach/cancel (2026-08-21):** job disparado por convidado vive no silo do dono — `/jobs/active` não o via (toast sumia após reload) e `/jobs/cancel-batch` devolvia `cancelled_count=0` em silêncio. Resolvido pelo par `payload.actor_id` (quem disparou) + grant no pack; `actor_id`/`silo_user_id` padronizados nos payloads de transcrição e sync (só refresh já os tinha). Grant revogado entre disparo e reload → o job some da lista. Cancel agrupa por silo: em silo alheio cancela quem **disparou** o job ou quem manda no pack (`dono|editor`); viewer não cancela trabalho alheio. Sintaxe do filtro JSON do PostgREST validada ao vivo (errar falharia silencioso). **Sub-fase concluída.** |
...
| P3.5 | Log de ações (ator, alvo, ação) + retenção de 365 dias | `Concluído` — 2026-08-22 (migration 114). **A razão de existir:** num pack compartilhado a credencial é do DONO, então o log do Gerenciador de Anúncios da Meta atribui ao dono tudo que o convidado faz — `pack_action_log` é o único lugar do mundo que sabe quem agiu. **Onde NÃO foi instrumentado, e por quê:** em `assert_pack_role`/`resolve_entity_pack_scope`, que pareciam o gancho perfeito por rodarem antes de toda escrita — mas são AUTORIZAÇÃO: registram tentativa, não resultado, e o de mídia roda em leitura. O registro é explícito por rota, depois do efeito, com `status` ok|error|partial (lote parcial da Meta guarda só os ids que de fato mudaram). **Fire-and-forget** (precedente do `meta_usage_logger`): log que falha nunca derruba a ação — e por isso `tests/conftest.py` neutraliza o pool, senão a suite escreveria no banco REAL (a primeira execução provou: as chamadas saíram e só não gravaram porque os uuids de fixture foram recusados). **Fora do vocabulário de propósito:** o status-sync on-focus (reconcilia cache, não muda nada na Meta, dispararia a cada troca de aba) e o cancel de job no próprio silo (é o logout). **Sem FK para packs**: "quem apagou o pack" é justamente uma das perguntas que o log responde, e um CASCADE apagaria a resposta. **Retenção** = função `purge_pack_action_log()` + pg_cron (jobid 1, 04:23 UTC) — a extensão só dispara; se ela sumir, a retenção vira chamada manual em vez de deixar de existir. **Leitura**: qualquer membro, viewer inclusive (todos já veem os EFEITOS; esconder o autor não protege nada), via `GET /analytics/packs/{id}/activity` com cursor, e `PackActivityDialog` no menu do PackCard. Provado ao vivo com a conta convidada disjunta: ator `269e9020`, silo `8363e117`, papel editor |
