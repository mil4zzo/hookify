...
| 2.1–2.7 | 2026-09-19 | **Migration 167** ganhou mais três funções: `pack_trim_head` (ausência só apaga na cabeça, e recusa lista vazia por exceção), `pack_prune_ad_ids` (devolve quem saiu; conta inventário, não só métrica) e `pack_recompute_conversion_types`. Teste `supabase/tests/167_recorte_de_periodo.test.sql`: 18 asserções, **5 sabotagens, todas pegam**. **Backend**: `supabase_repo.trim_pack_to_window` orquestra na ordem do §2.3 (cabeça → período dia a dia + varredura → inventário → anúncios pelo mesmo mecanismo do `delete_pack` → conversion_types), sem tocar nas datas; `pack_trim_preview` e `GET /packs/{id}/date-range/preview` (dono). Rota: redução SEM fatia resolve síncrona (medido ~1,7 s) e devolve `status: completed`; com fatia, o job recorta antes de `apply_pack_window_edit`. **Frontend**: diálogo aceita reduzir, prévia com debounce de 300 ms, `InlineNotice` de aviso com dias/investimento, botão destrutivo "Alterar e apagar"; hook trata a resposta síncrona. **Testes**: `test_trim_pack_window` (7), mais os da rota e do job — 910 no backend, `tsc` e design-system limpos; 5 sabotagens no Python, todas pegam. **Dois bugs meus achados na revisão**: a checagem de órfãos olhava só os 200 primeiros anúncios (passou a usar o mecanismo do `delete_pack`), e a leitura dos dias fora do período batia no teto silencioso de 1000 linhas do PostgREST (passou a `_fetch_all_paginated`, só as linhas fora). **Pendente**: ~~migrations 166 e 167 não aplicadas em produção~~ (aplicadas em 21/09 às pressas, depois que o deploy de 20/09 subiu o backend sem elas e derrubou toda atualização de pack); teste manual na conta disjunta. |
