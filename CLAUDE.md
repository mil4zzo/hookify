...
**Design system (OBRIGATÓRIO ao criar/editar UI)**: antes de criar qualquer componente visual, leia `documentation/authenticated-app-visual-standard.md`. Regras inegociáveis, todas medidas — não improvise variação:

- **Superfície sai da escada de cinco níveis** (`background` 0 · `card`/`muted`/`popover` 1 · `surface-2` 2 · `input`/`surface-3` 3 · `border` 4). Regra mecânica: **onde se age, sobe um nível; onde se lê, desce um**; vizinhos nunca no mesmo nível sem borda. Desempate quando há leitura e ação: rolagem ou volume de conteúdo desce, linha que é um formulário sobe. NUNCA pinte superfície com a escala `-N` (nem em parada de gradiente): ela mistura com a página e inverte de sinal entre os temas.
- **A escala `-N` é só tinta** (`bg-primary-10`, `border-warning-30`) e é alpha real. **Preenchimento sólido de botão usa o passo escuro** da cor (`success-800`, `destructive-600`), porque o tom de texto reprova no contraste com branco.
- **Física da interface:** `btn-lit` = clicável (luz de cima), `btn-tonal` = secundário, `state-pressed` = **já acionado** (luz invertida: aba ativa, item do menu da página atual, dia do calendário, opção escolhida) e `control-lit` / `<Button variant="picker">` = **gatilho** (abre lista para escolher ou filtrar: borda do campo + degradê, sem sombra). Botão executa ação e tem sombra; gatilho guarda valor e não tem. Campo de texto é chapado.
- **Foco é sempre desenhado POR DENTRO**: campo usa a própria borda (`focus:border-primary`); o resto usa a classe `focus-inset`. Anel externo (`focus:ring-2`, `ring-offset-*`) é proibido — qualquer container com rolagem corta os 4px.
- **Altura de controle: 40px, uma só**, sempre via prop `size` (`sm` é a mesma altura com menos respiro lateral), NUNCA via `h-8`/`h-9`/`h-10` em className. Exceções: chip de uma palavra (24px) e aba (32px). Grupo horizontal de botões: `gap-3`.
- **Tipografia:** dois tons de texto (`foreground`, `muted-foreground`; apagado = `opacity-50`). `text-2xs` (10px) só em rótulo em maiúsculas, selo e dado em célula densa — **frase é `text-xs`**. Nunca `text-[Npx]`. Raio por papel: `rounded-sm` pastilha, `rounded-md` controle e bloco, `rounded-lg` cartão e modal, `rounded-full` pílula. Borda: um token (`border-border`). Sombras: só `shadow-elevation-flat/raised/overlay`. Camadas: só `z-sticky/overlay/modal/dropdown/toast/tooltip`.
- **Estado vazio é sempre `StatePanel`** (empilhado, nunca card elevado). Moldura pelo lugar: `frame="dashed"` no lugar de lista/grade no corpo da página, `none` dentro de card/modal/tabela, `layout="inline"` em linha de tabela; erro nunca tracejado. Tracejado tem dois donos: estado vazio (apagado) e área que recebe arrasto (borda padrão, `primary` no arrasto).
- **Tailwind 3.4, não 4.** Quatro jeitos de uma classe não gerar CSS e sumir calada: barra de opacidade em token do tema (`bg-destructive/5`), sintaxe v4 (`aria-invalid:`, `rounded-xs`), passo fora da escala e `var()` inexistente. Ao colar componente do shadcn, converta antes.
...
- `npm run check:design-system` + `tsc --noEmit` rodam no pre-commit (husky). O checker cobre o que é mecânico; nível certo de superfície e papel do texto são julgamento seu. Exceção pontual: `// design-system-exception: rule-id - razão` na linha acima da violação.
