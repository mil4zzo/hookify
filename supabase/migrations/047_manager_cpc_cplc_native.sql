-- Migration 047
-- Padroniza CPC e CPLC na camada nativa de dados do Manager.
-- - Enriquecimento do payload agregado com cpc/cplc
-- - Exposição de clicks/inline_link_clicks/cpc/cplc na RPC de séries

do $$
begin
  if to_regprocedure('public.fetch_manager_analytics_aggregated_base_v047(uuid,date,date,text,uuid[],text[],text,text,text,boolean,boolean,integer,integer,text)') is null then
    ...
    ) rename to fetch_manager_analytics_aggregated_base_v047;
  end if;
end;
$$;
...
'Manager aggregated RPC wrapper: enriches the payload with native cpc/cplc values and averages while preserving the existing contract.';
...
'Manager series v2 RPC: returns sparkline series for requested group_keys, including clicks, inline_link_clicks, cpc and cplc, plus retention metrics.';
