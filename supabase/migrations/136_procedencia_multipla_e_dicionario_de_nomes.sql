-- =====================================================================
-- 136 - Procedencia MULTIPLA na linha + dicionario de nomes na resposta
-- =====================================================================
--
-- O PROBLEMA, NA PRATICA
--   Uma linha do Manager agrupada por criativo colapsa dezenas de anuncios, que
--   podem estar espalhados por varias campanhas. `campaign_name` na linha e o do
--   REPRESENTANTE (o anuncio de maior entrega) - e filtrar por ele acerta a
--   maioria e MENTE no resto, em silencio. Medido no laboratorio, na conta
--   principal: 390 de 1.739 criativos (22%) rodam em mais de uma campanha.
--
--   "Me mostre os criativos da campanha X" hoje esconde todo criativo cuja maior
--   entrega esteja em outra campanha. Nao da erro: some da tabela.
--
-- O QUE MUDA
--   A linha passa a poder trazer `campaign_ids` e `adset_ids` - TODOS os pais do
--   grupo, como `account_ids` e `pack_ids` ja fazem - e a resposta ganha `names`,
--   um dicionario id -> nome na raiz. Com isso a pergunta vira "ALGUMA campanha do
--   criativo e X", que e o que o usuario quer dizer.
--
-- POR QUE E OPT-IN (`p_include_parent_ids`, default false)
--   Porque custa, e o plano mandou medir antes de decidir. Medido no laboratorio,
--   em bytes COMPRIMIDOS (que e o que viaja):
--
--     criativos | 36 packs | 13 meses : 224 kB -> 373 kB  (+67%)
--     criativos | 36 packs | 30 dias  :  85 kB -> 115 kB  (+35%)
--     anuncios / conjuntos / campanhas:                    (+6%)
--
--   O peso e todo nas abas de CRIATIVO, onde a linha agrega muitos anuncios: 23
--   campanhas e 60 conjuntos por linha no recorte de 13 meses. Nas outras abas cada
--   linha tem um pai so.
--
--   Desligado, a resposta e IDENTICA a da v132 - nem uma chave a mais, e o `filter`
--   do array_agg nem acumula. Quem liga: Manager e Boards (o filtro de campanha vive
--   la) e o Plano/GOLD/Insights QUANDO o criterio de validacao cita campanha ou
--   conjunto - senao a condicao existiria e seria ignorada em silencio, que e
--   exatamente o bug de campo morto que a fase 4 acabou de matar.
--
--   Nao e a forma `p_x is null or ...` que degradou 270x na 6a execucao da conexao
--   (generic plan): e um booleano constante, e a funcao ja roda com
--   `plan_cache_mode = force_custom_plan`.
--
-- POR QUE O NOME NAO VIAJA NA LINHA
--   Nome de campanha e longo e se repete: 500 linhas x 23 campanhas seriam ~11 mil
--   copias de algumas centenas de nomes. O id (17 caracteres) viaja; o nome vem uma
--   vez so, e so dos ids que aparecem nas linhas DESTA pagina.
--
--   O fold final (`p_campaign_id` filtra a pagina DEPOIS de montada) poda o
--   dicionario junto: sem isso ele carregaria os nomes de todos os pais da pagina
--   inteira. O diferencial mediu a sobra antes da correcao - 2.011 conjuntos, ~186 kB
--   numa resposta de poucas linhas. Nenhum caller do app envia esse filtro hoje, mas
--   quem o ligar amanha nao herda a armadilha.
--
-- ONDE O NOME MORA
--   Em `public.ads` (campaign_name/adset_name), nao em `parent_entities` - que tem
--   `entity_id` mas nenhuma coluna de nome. `supabase/schema_map.md` esta parado
--   numa versao anterior e nao mostra essas colunas; a fonte foi o banco.
--
-- A ENTRADA PRECISA DE DROP, NAO DE REPLACE
--   `fetch_manager_rankings_core_v2` ganha o 17o parametro. `create or replace` nao
--   adiciona parametro: criaria uma SEGUNDA sobrecarga, e o PostgREST passaria a nao
--   saber qual chamar (PGRST203). O `drop` da versao de 16 argumentos vem antes.
--
-- COMPATIBILIDADE
--   `campaign_name`/`adset_name` do representante CONTINUAM na linha - a limpeza e
--   da fase 6, quando nenhum consumidor os ler. Com o gate desligado todas as chaves
--   sao identicas as da v132, e com ele ligado so ha acrescimo (provado pelo
--   diferencial `--v136` do `backend/scripts/diff_rankings_rollup.py`).
--
-- GERADA a partir do corpo real da v132 lido do banco (nao transcrita a mao).
-- =====================================================================

begin;

CREATE OR REPLACE FUNCTION public.fetch_manager_performance_base_v136(p_user_id uuid, p_date_start date, p_date_stop date, p_group_by text DEFAULT 'ad_name'::text, p_pack_ids uuid[] DEFAULT NULL::uuid[], p_account_ids text[] DEFAULT NULL::text[], p_campaign_name_contains text DEFAULT NULL::text, p_adset_name_contains text DEFAULT NULL::text, p_ad_name_contains text DEFAULT NULL::text, p_action_type text DEFAULT NULL::text, p_include_leadscore boolean DEFAULT true, p_include_available_conversion_types boolean DEFAULT true, p_limit integer DEFAULT 500, p_offset integer DEFAULT 0, p_order_by text DEFAULT 'spend'::text, p_campaign_id text DEFAULT NULL::text, p_include_parent_ids boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- A entrada ganha o 17o parametro e o repassa. `drop` primeiro: `create or replace`
-- criaria uma segunda sobrecarga de 16 argumentos e o PostgREST nao saberia qual
-- chamar. Fora isso, e copia literal do wrapper em producao.
...
declare
  ...
  v_data jsonb := '[]'::jsonb;
begin
  select public.fetch_manager_performance_base_v136(
    ...
    p_order_by, p_campaign_id, p_include_parent_ids
  )
  into v_payload;
...
begin
  ...
  if v_sobrecargas is distinct from 1 then
    raise exception '136: a entrada ficou com % sobrecargas - o PostgREST nao saberia qual chamar', v_sobrecargas;
  end if;
  ...
  if v_alvo is distinct from 'fetch_manager_performance_base_v136' then
    raise exception '136: a entrada continua apontando para %', coalesce(v_alvo, '<nenhuma>');
  end if;
  raise notice '136 OK: fetch_manager_rankings_core_v2 (1 sobrecarga) -> fetch_manager_performance_base_v136';
end $$;
