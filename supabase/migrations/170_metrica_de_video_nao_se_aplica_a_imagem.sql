-- ===========================================================================
-- 170 — MÉTRICA DE VÍDEO NÃO SE APLICA A ANÚNCIO DE IMAGEM
--
-- A Meta às vezes manda métrica de vídeo num anúncio de imagem. Na cópia de
-- produção de 08/09: 82 anúncios de imagem e 572 linhas-dia com plays, 5.554 plays
-- no total (0,33% das linhas). É pouco volume e muito estrago — um estático com
-- 1 play e "hook de 100%" vencia o ranking de hook, e no grão de campanha os plays
-- espúrios entram no denominador de todo mundo.
--
-- A REGRA MORA NA LEITURA, NÃO NA ESCRITA. `ad_metrics` continua sendo a cópia fiel
-- do que a Meta disse: (a) é o que permite auditar um número; (b) a classificação de
-- mídia USA plays como prova ("tem play => é vídeo", ad_media.resolve_media_type,
-- passo 3) — podar na ingestão apagaria o sinal que corrige um anúncio classificado
-- errado, e ele ficaria imagem para sempre; (c) o tipo de mídia é do ANÚNCIO e muda
-- com o tempo, a linha de métrica é do DIA e é histórico.
--
-- O QUE MUDA: a v170 é a v162 com a trava no passo 5 (anúncio → grupo). Vale para
-- todos os grãos e para os totais do cabeçalho, que descem do mesmo lugar.
--
-- O ÍNDICE FAZ PARTE DA MUDANÇA, não é acessório: o passo 4 lê `ads` por anúncio e
-- hoje cai inteiro no índice de cobertura. Pedir `media_type` sem ele no índice fez o
-- planejador abandonar o índice e varrer a tabela — medido no laboratório: 5.323
-- páginas (index-only, 0 heap fetches) contra ~21 mil (Seq Scan). Com `media_type`
-- dentro do índice: 5.406 páginas, 0 heap fetches, e o índice continua com 12 MB.
--
-- VOLTA ATRÁS sem deploy: `ANALYTICS_MANAGER_COLUMNS_RPC=fetch_manager_rankings_v162`
-- (a v162 fica intacta). O índice novo serve às duas.
--
-- AS SETE COLUNAS, NÃO SÓ O DENOMINADOR: travar apenas `plays` parece equivalente
-- (toda razão divide por plays), mas num nome MISTURADO o hook sobe — o play da imagem
-- sai do denominador e o hook dela fica no numerador. Provado por sabotagem: 0,6 em vez
-- de 0,5 (supabase/tests/170_video_em_imagem.test.sql, C1).
--
-- SÓ `image`, nunca `unknown`: "não sei o formato" não é "é imagem". Tratar `unknown`
-- como imagem apagaria métrica de vídeo de um anúncio cuja classificação apenas falhou.
-- (Na cópia de 08/09 isso não apagaria nada hoje: nenhum dos 1.812 sem formato tem play,
-- e só 2 tiveram entrega. É regra de princípio, não de volume — e o teste sintético é
-- quem a guarda, porque o dado real não a exercita.)
--
-- PROVA: supabase/tests/170_video_em_imagem.test.sql (casos que o dado real não tem)
-- e backend/scripts/diff_manager_v170.py (v162 × v170 nos mesmos cenários do
-- diferencial da 161/162: só métrica de vídeo de anúncio de imagem pode mudar).
-- ===========================================================================

-- 1. O índice de cobertura passa a levar media_type.
--
-- FORA DE TRANSAÇÃO, E CONCURRENTLY — de propósito. `CREATE INDEX` comum trava a
-- escrita em `public.ads` (o refresh de pack escreve lá), e `DROP INDEX` pega lock
-- exclusivo que, DENTRO de um BEGIN, só sai no COMMIT: com a função de 1.300 linhas
-- no meio e o teto de 40 s da RPC do Manager (migration 164), uma consulta em voo
-- enfileiraria o app inteiro. A migration 124 já criava este índice assim; a 132
-- quebrou o padrão e esta migration repetia o erro até a revisão de 20/09.
--
-- CONCURRENTLY não roda dentro de transação: se o seu runner envolver o arquivo num
-- BEGIN, rode ESTE trecho por psql direto, e só depois o resto. Se o CREATE falhar no
-- meio, sobra um índice INVÁLIDO — derrube-o (`DROP INDEX CONCURRENTLY
-- ads_user_ad_status_mt_idx`) e rode de novo.
SET lock_timeout = '5s';

CREATE INDEX CONCURRENTLY IF NOT EXISTS ads_user_ad_status_mt_idx
  ON public.ads (user_id, ad_id)
  INCLUDE (effective_status, meta_created_time, thumb_storage_path, media_type);

DROP INDEX CONCURRENTLY IF EXISTS public.ads_user_ad_status_idx;

RESET lock_timeout;

COMMENT ON INDEX public.ads_user_ad_status_mt_idx IS
  'Cobre o lookup por anúncio da RPC do Manager (passo 4): status, criação, miniatura e
tipo de mídia sem tocar na tabela. media_type entrou na 170 — sem ele a leitura deixa de
ser index-only e vira varredura.';

BEGIN;

-- 2. A v170: a v162 com a regra.
CREATE OR REPLACE FUNCTION public.fetch_manager_rankings_v170(p_user_id uuid, p_date_start date, p_date_stop date, p_group_by text DEFAULT 'ad_name'::text, p_pack_ids uuid[] DEFAULT NULL::uuid[], p_account_ids text[] DEFAULT NULL::text[], p_campaign_name_contains text DEFAULT NULL::text, p_adset_name_contains text DEFAULT NULL::text, p_ad_name_contains text DEFAULT NULL::text, p_action_type text DEFAULT NULL::text, p_include_leadscore boolean DEFAULT true, p_include_available_conversion_types boolean DEFAULT true, p_limit integer DEFAULT 500, p_offset integer DEFAULT 0, p_order_by text DEFAULT 'spend'::text, p_campaign_id text DEFAULT NULL::text, p_include_custom boolean DEFAULT false, p_thumb_public_prefix text DEFAULT NULL::text)
 RETURNS SETOF json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.fetch_manager_rankings_v170(uuid, date, date, text, uuid[], text[], text, text, text, text, boolean, boolean, integer, integer, text, text, boolean, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.fetch_manager_rankings_v170(uuid, date, date, text, uuid[], text[], text, text, text, text, boolean, boolean, integer, integer, text, text, boolean, text) TO authenticated, service_role;

COMMENT ON FUNCTION public.fetch_manager_rankings_v170(uuid, date, date, text, uuid[], text[], text, text, text, text, boolean, boolean, integer, integer, text, text, boolean, text) IS
  '170: a v162 com a regra "métrica de vídeo não se aplica a anúncio de imagem" — anúncio com media_type=image não contribui para plays, thruplays, hook, hold rate, scroll stop, 50% e 75% (nem nas linhas, nem nos totais). Resto idêntico à v162.';

-- Prova no próprio arquivo: índice, configuração, permissões e forma de retorno.
DO $$
DECLARE
  ...
  v_col text;
BEGIN
  SELECT pg_get_indexdef('public.ads_user_ad_status_mt_idx'::regclass) INTO v_def;
  IF v_def IS NULL OR position('media_type' in v_def) = 0 THEN
    RAISE EXCEPTION '170: o índice de cobertura não leva media_type. def=%', v_def;
  END IF;
  IF to_regclass('public.ads_user_ad_status_idx') IS NOT NULL THEN
    RAISE EXCEPTION '170: o índice antigo continua lá — duas cópias do mesmo índice';
  END IF;

  SELECT * INTO v_proc FROM pg_proc WHERE oid = 'public.fetch_manager_rankings_v170(uuid, date, date, text, uuid[], text[], text, text, text, text, boolean, boolean, integer, integer, text, text, boolean, text)'::regprocedure;
  IF v_proc.proconfig IS NULL OR NOT ('plan_cache_mode=force_custom_plan' = ANY(v_proc.proconfig)) THEN
    RAISE EXCEPTION '170: sem plan_cache_mode. proconfig=%', v_proc.proconfig;
  END IF;
  IF NOT v_proc.proretset OR v_proc.prorettype <> 'json'::regtype THEN
    RAISE EXCEPTION '170: a função deveria ser SETOF json (como a v162)';
  END IF;
  IF NOT v_proc.prosecdef THEN
    RAISE EXCEPTION '170: a função deveria ser SECURITY DEFINER';
  END IF;
  IF has_function_privilege('anon', 'public.fetch_manager_rankings_v170(uuid, date, date, text, uuid[], text[], text, text, text, text, boolean, boolean, integer, integer, text, text, boolean, text)', 'EXECUTE') THEN
    RAISE EXCEPTION '170: anon consegue executar a v170';
  END IF;
  IF (SELECT count(*) FROM pg_proc WHERE proname = 'fetch_manager_rankings_v162') <> 1 THEN
    RAISE EXCEPTION '170: a v162 sumiu — ela é a volta atrás sem deploy';
  END IF;

  -- As SETE travas, UMA A UMA. Não basta contar ocorrências: a revisão de 20/09 provou
  -- por sabotagem que apagar UMA delas (hold_rate) passava por todas as provas.
  SELECT prosrc INTO v_def FROM pg_proc WHERE proname = 'fetch_manager_rankings_v170';
  FOREACH v_col IN ARRAY ARRAY['plays', 'thruplays', 'hook_wsum', 'hold_rate_wsum',
                               'video_watched_p50_wsum', 'video_watched_p75_wsum', 'scroll_stop_wsum'] LOOP
    IF position(format('case when p.media_type = ''image'' then 0 else p.%s end', v_col) in v_def) = 0 THEN
      RAISE EXCEPTION '170: a coluna % ficou sem trava de imagem', v_col;
    END IF;
  END LOOP;
END;
$$;
