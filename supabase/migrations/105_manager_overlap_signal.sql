-- Migration 105: sinal de conflito cross-silo no payload do Manager (P3.2, camada 2)
--
-- O dedup da 104 evita a dupla contagem, mas em SILENCIO: o usuario ve um numero
-- que escolheu arbitrariamente entre dois silos. Esta migration acende a luz.
--
-- Payload ganha `overlap: { rows: N }`, AUSENTE quando nao ha conflito — o
-- frontend nao precisa saber de nada no caso comum.
--
-- POR QUE O SINAL OLHA O DONO, NAO A DUPLICIDADE DE LINHA
--   Dois packs DO MESMO usuario podem compartilhar anuncios. Nao e hipotese:
--   existe no banco hoje um par com 5.639 linhas em comum. Isso e benigno e nada
--   tem a ver com compartilhamento. Se o sinal fosse "linha duplicada -> avisa",
--   o alerta dispararia para quem nunca compartilhou nada e seria aprendido como
--   ruido. Por isso a condicao e "veio de mais de um DONO".
--
-- CUSTO ZERO: as duas agregacoes de janela pegam carona na ordenacao
-- (ad_id, date) que o dedup da 104 ja faz. Nenhuma ordenacao nova.
--
-- DETALHE: uuid nao tem agregado min/max no Postgres, e
-- `count(distinct ...) over (...)` nao existe. O par min/max sobre user_id::text
-- e o jeito de perguntar "ha mais de um valor distinto nesta janela?".
--
-- NAO devolve quais packs conflitam: nomear exigiria um join a mais no caminho
-- mais quente do app, e quem nomeia e o grafo de conflitos da camada 1, que ja
-- existe para isso. A RPC e o fio de tropeco — diz que aconteceu e o tamanho.
--
-- v104 e MANTIDA. Reverter e trocar uma linha no wrapper. Diferencial rodado
-- antes da troca: v105 == v104 em 3 group_by; overlap ausente sem conflito;
-- silencioso na sobreposicao do mesmo dono; e {"rows": 5} num cenario sintetico
-- com exatamente 5 linhas duplicadas entre silos.

BEGIN;

-- Base com sinal de conflito
CREATE OR REPLACE FUNCTION public.fetch_manager_rankings_core_v2_base_v105(p_user_id uuid, p_date_start date, p_date_stop date, p_group_by text DEFAULT 'ad_name'::text, p_pack_ids uuid[] DEFAULT NULL::uuid[], p_account_ids text[] DEFAULT NULL::text[], p_campaign_name_contains text DEFAULT NULL::text, p_adset_name_contains text DEFAULT NULL::text, p_ad_name_contains text DEFAULT NULL::text, p_action_type text DEFAULT NULL::text, p_include_leadscore boolean DEFAULT true, p_include_available_conversion_types boolean DEFAULT true, p_limit integer DEFAULT 500, p_offset integer DEFAULT 0, p_order_by text DEFAULT 'spend'::text, p_campaign_id text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- Wrapper vivo passa a apontar para a v105
...
BEGIN
  ...
  IF v_base IS DISTINCT FROM 'core_v2_base_v105' THEN
    RAISE EXCEPTION 'ABORTADO: wrapper delega a %, esperava core_v2_base_v105.', coalesce(v_base,'(nenhuma)');
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE pronamespace='public'::regnamespace
                 AND proname='fetch_manager_rankings_core_v2_base_v104') THEN
    RAISE EXCEPTION 'ABORTADO: v104 sumiu — sem caminho de rollback.';
  END IF;
END
$post$;
