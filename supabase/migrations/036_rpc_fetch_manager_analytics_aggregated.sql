-- Migration 036
-- RPC otimizada para o Manager:
-- - filtra e agrega no banco (evita tráfego de linhas brutas)
-- - devolve payload já no contrato esperado (data + averages + available_conversion_types)
-- - gera séries diárias apenas na janela final (ex.: últimos 7 dias)

create or replace function public.fetch_manager_analytics_aggregated(
  p_user_id uuid,
  p_date_start date,
  p_date_stop date,
  p_group_by text default 'ad_name', -- ad_id | ad_name | adset_id | campaign_id
  ...
  p_include_series boolean default true,
  p_include_leadscore boolean default true,
  p_series_window integer default 7,
...
begin
  ...
  select
    ...
    coalesce(
      (
        ...
        from unnest(coalesce(am.leadscore_values, '{}'::numeric[])) v
        where v >= v_mql_min
      ),
      0
    ) as mql_count_row
  ...
  drop table if exists pg_temp.mgr_conv_group;
  create temporary table pg_temp.mgr_conv_group on commit drop as
  select
    group_key,
    conv_key,
  ...
  from (
    select
      g.group_key,
      unnest(coalesce(g.ad_ids, array[]::text[])) as ad_id
  ...
  if p_include_series then
    ...
    select
      ...
      jsonb_build_object(
        'axis', (
          select jsonb_agg(to_char(a.d, 'YYYY-MM-DD') order by a.d)
          from pg_temp.mgr_axis a
        ),
        'hook', (
          select jsonb_agg(
            case when d.plays > 0 then d.hook_wsum / d.plays else null end
            order by a.d
          )
          from pg_temp.mgr_axis a
        ...
        'spend', (
          select jsonb_agg(
            case when coalesce(d.spend, 0) <> 0 then d.spend else null end
            order by a.d
          )
          from pg_temp.mgr_axis a
          left join pg_temp.mgr_daily d
        ...
        'ctr', (
          select jsonb_agg(
            case when d.impressions > 0 then d.clicks::numeric / d.impressions else null end
            order by a.d
          )
          from pg_temp.mgr_axis a
          left join pg_temp.mgr_daily d
        ...
        'connect_rate', (
          select jsonb_agg(
            case when d.inline_link_clicks > 0 then d.lpv::numeric / d.inline_link_clicks else null end
            order by a.d
          )
          from pg_temp.mgr_axis a
          left join pg_temp.mgr_daily d
        ...
        'impressions', (
          select jsonb_agg(
            case when coalesce(d.impressions, 0) <> 0 then d.impressions else null end
            order by a.d
          )
          from pg_temp.mgr_axis a
          left join pg_temp.mgr_daily d
        ...
        'cpm', (
          select jsonb_agg(
            case when d.impressions > 0 then (d.spend * 1000.0) / d.impressions else null end
            order by a.d
          )
          from pg_temp.mgr_axis a
          left join pg_temp.mgr_daily d
        ...
        'website_ctr', (
          select jsonb_agg(
            case when d.impressions > 0 then d.inline_link_clicks::numeric / d.impressions else null end
            order by a.d
          )
          from pg_temp.mgr_axis a
          left join pg_temp.mgr_daily d
        ...
        'cpmql', (
          select jsonb_agg(
            case
              when coalesce(d.mql_count, 0) > 0 and coalesce(d.spend, 0) > 0
              then d.spend / d.mql_count
              else null
            end
            order by a.d
          )
...
grant execute on function public.fetch_manager_analytics_aggregated(
...
'RPC otimizada do Manager: agrega métricas por group_by (ad_id/ad_name/adset_id/campaign_id), retorna séries diárias (janela final), médias globais e tipos de conversão sem transferir linhas brutas.';
