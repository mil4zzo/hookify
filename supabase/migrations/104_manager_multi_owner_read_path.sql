-- Migration 104: Manager le packs de MULTIPLOS donos (P3.2)
--
-- Fecha o read-path do compartilhamento: a RPC principal do Manager passa a
-- derivar os DONOS do dado a partir de p_pack_ids, em vez de filtrar por um
-- user_id escalar. Packs de donos diferentes convivem numa unica agregacao —
-- uma query, uma media.
--
-- ASSINATURA PRESERVADA. O roadmap previa remover p_user_id; nao foi feito de
-- proposito: mudar a lista de parametros cria ambiguidade de overload no
-- PostgREST, que e exatamente a armadilha que a 095 teve de limpar. p_user_id
-- passa a significar o ATOR (quem pergunta) e segue validado contra auth.uid();
-- o argumento do roadmap ("nao ha id para forjar") ja era atendido por esse guard.
--
-- MEDICOES QUE DEFINIRAM O DESENHO (EXPLAIN ANALYZE, dados reais, 90 dias)
--   hoje, um dono, igualdade escalar ............................  67 ms
--   `am.user_id = any(v_owners)` (desenho ingenuo) .............. 4010 ms  (60x pior)
--   dirigido pelos donos -> map -> metrics ......................  11 ms  (6x melhor)
--   dois silos, 22.9k linhas ....................................  197 ms
--
-- O desenho ingenuo faz o planner perder ad_metric_pack_map_user_pack_date_ad_idx
-- e cair no PK varrendo todos os user_ids. Dirigindo a partir dos donos, o nested
-- loop liga as 4 colunas do indice composto. Ou seja: a mudanca de correcao
-- tambem ACELERA o caminho de dono unico que ja existia.
--
-- SEGURANCA: pack inacessivel nao volta de resolve_pack_access, e a contagem
-- denuncia -> Forbidden. Falhar alto e melhor que devolver agregado
-- silenciosamente incompleto.
--
-- DEDUP CROSS-SILO: o mesmo anuncio/dia pode existir em dois silos quando o
-- convidado tambem carregou os mesmos anuncios (o passivo do jeito antigo).
-- distinct on (ad_id, date) conta uma vez so; vence o silo do DONO do pack
-- compartilhado, desempate por uuid. Custo zero: o Postgres ja eliminava user_id
-- da chave de ordenacao por ser constante.
--
-- v093 e MANTIDA. Reverter e trocar uma linha no wrapper. Diferencial rodado
-- antes da troca: v104 == v093 em 7 combinacoes (ad_name/ad_id/adset_id/
-- campaign_id, 30d e 90d, ramo legado sem packs, e com action_type).

BEGIN;

-- Nova base multi-dono
CREATE OR REPLACE FUNCTION public.fetch_manager_rankings_core_v2_base_v104(p_user_id uuid, p_date_start date, p_date_stop date, p_group_by text DEFAULT 'ad_name'::text, p_pack_ids uuid[] DEFAULT NULL::uuid[], p_account_ids text[] DEFAULT NULL::text[], p_campaign_name_contains text DEFAULT NULL::text, p_adset_name_contains text DEFAULT NULL::text, p_ad_name_contains text DEFAULT NULL::text, p_action_type text DEFAULT NULL::text, p_include_leadscore boolean DEFAULT true, p_include_available_conversion_types boolean DEFAULT true, p_limit integer DEFAULT 500, p_offset integer DEFAULT 0, p_order_by text DEFAULT 'spend'::text, p_campaign_id text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- Wrapper vivo passa a apontar para a v104
...
declare
  ...
  v_data jsonb := '[]'::jsonb;
begin
  select public.fetch_manager_rankings_core_v2_base_v104(
    p_user_id,
    p_date_start,
    p_date_stop,
    p_group_by,
...
-- Pos-condicao: o wrapper vivo delega a v104 e a v093 continua disponivel para reverter.
DO $post$
DECLARE v_base text;
BEGIN
  ...
  IF v_base IS DISTINCT FROM 'core_v2_base_v104' THEN
    RAISE EXCEPTION 'ABORTADO: wrapper delega a %, esperava core_v2_base_v104.', coalesce(v_base,'(nenhuma)');
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE pronamespace='public'::regnamespace
                 AND proname='fetch_manager_rankings_core_v2_base_v093') THEN
    RAISE EXCEPTION 'ABORTADO: v093 sumiu — sem caminho de rollback.';
  END IF;
END
$post$;
