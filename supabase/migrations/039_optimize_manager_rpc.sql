-- Migration 039
-- Otimização do RPC fetch_manager_analytics_aggregated.
-- Correções baseadas em diagnóstico real (diagnose_manager_rpc_timing):
--
-- 1. mgr_series: troca 12 subqueries correlacionadas por CROSS JOIN + jsonb_agg
--    → ad_id com 5096 grupos: 67s → <1s
-- 2. mgr_base: remove contagem MQL per-row (unnest+count em 13K+ linhas)
--    → contagem movida para mgr_daily (só janela de séries)
-- 3. mgr_status: usa DISTINCT de mgr_base ao invés de unnest de array agrupado
--    → evita materializar array de ad_ids no mgr_group
-- 4. mgr_group: remove array_agg(distinct ad_id) (não mais necessário)
-- 5. mgr_conv_daily_map: adiciona índice para JOIN da série
--
-- Contrato de saída (JSON) é 100% idêntico à versão anterior.
...
-- Reutiliza o GRANT existente (mesma assinatura)
...
'RPC otimizada do Manager v2: agrega métricas por group_by, séries via CROSS JOIN (sem subqueries correlacionadas), MQL calculado só na janela de séries.';
