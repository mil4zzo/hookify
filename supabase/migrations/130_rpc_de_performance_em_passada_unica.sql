-- 130: RPC do Manager em passada única sobre o read model (fase B do plano).
--
-- ATENÇÃO — CUTOVER: esta migration cria a base nova E reponta a entry
-- `fetch_manager_rankings_core_v2` para ela. Aplicar só junto com o deploy do
-- backend/frontend que entende `leadscore_histogram` (fase D/E do plano). Rollback:
--
--   CREATE OR REPLACE FUNCTION public.fetch_manager_rankings_core_v2 ... (corpo da
--   migration 122, chamando ..._base_v116)  — ou simplesmente re-aplicar a 122.
--
-- Pré-requisito: migrations 128 e 129 aplicadas e backfill concluído
-- (ad_performance_rollup_consistency_check(user) = zero linhas para todo usuário).
--
-- O QUE MUDA E POR QUÊ (medido em 2026-08-26, ver documentation/plano-rollup-rankings.md)
-- ---------------------------------------------------------------------------------------
-- A base v116 lia `select am.*` (linhas de ~1 KB, 40% JSON), deduplicava 64 mil linhas
-- largas com DISTINCT ON + janela, releia a CTE `filtered` em 8 etapas, abria o JSON de
-- conversões elemento a elemento e explodia `leadscore_values` para reconcatenar por
-- grupo: 2,5 s de CPU e 470 MB de arquivos temporários por carga do Manager.
--
-- Esta base:
--   1. `keys`     — resolve a SELEÇÃO no mapa de packs (índice composto, linhas de
--                   ~40 B): dedup cross-silo, sinal de conflito e a máscara de packs
--                   por anúncio-dia, tudo num GROUP BY estreito. Nunca ordena linha larga.
--   2. `sel`      — lê SÓ o read model (ad_performance_daily, migration 129) pela chave
--                   única: números já saneados, chaves de agrupamento e a conversão
--                   pedida = `conv_values[array_position(conv_key_ids, id)]`. Nenhuma
--                   linha de ad_metrics (~1 KB, JSON inline) é lida aqui.
--   3. `per_ad`   — agrega anúncio-dia → anúncio (silo, anúncio). Aqui morre o volume:
--                   42 mil linhas viram ~13 mil (packs com muito anúncio repetido). Só
--                   agregados de estado constante (somas, bit_or da máscara,
--                   max(chave do representante)) — nada de DISTINCT nem JSON.
--   4. `per_ad_status` — UM lookup em `ads` por anúncio (antes: por anúncio-dia).
--   5. `grp`      — anúncio → grupo (~77 linhas). Os DISTINCTs (contagens, contas)
--                   rodam sobre 1,5 mil linhas, não 64 mil.
--   6. resto (totais, médias, ordenação, paginação, filtro por campanha) idêntico à
--                   v116, sobre as ~77 linhas.
--
-- CONTRATO DE SAÍDA: os mesmos 5 blocos e os mesmos campos por linha da v116, com UMA
-- diferença: `leadscore_values` (array cru) vira `leadscore_histogram` ({score: qty}).
-- Provado pelo teste diferencial (backend/scripts/diff_rankings_rollup.py, fase C).
--
-- Regras herdadas (não negociáveis): SET plan_cache_mode + SET search_path aqui na
-- CREATE FUNCTION; entrada = ator, donos via resolve_pack_access; consulta dirigida
-- pelo mapa (nunca `am.user_id = any(...)`).

-- ============================================================================
-- 1. Representante do grupo: argmax por (impressões, ad_id) numa única passada
-- ============================================================================
-- A v116 achava o representante com DISTINCT ON sobre a CTE larga (segunda leitura +
-- ordenação). Aqui é um `max(text)` nativo sobre uma chave codificada:
--
--   lpad(impressions, 12, '0') ‖ US ‖ ad_id ‖ US ‖ user_id ‖ US ‖ campos do representante
--
-- comparada em COLLATE "C" (byte a byte). Como todo ad_id é numérico (verificado:
-- zero exceções), a ordem é exatamente (impressions desc, ad_id desc) da v116, e os
-- campos carona nunca influenciam (a chave já é única antes deles). NULL vira o
-- marcador RS (\x1e) para não se confundir com string vazia na saída.
--
-- Por que não um agregado sobre tipo composto: a primeira versão usava
-- max(row(...)::tipo) com SFUNC em SQL — ~55 mil chamadas de função por carga,
-- cada uma com custo de executor. max(text) é uma comparação em C.
--
-- work_mem: os únicos nós que ordenam/hasheiam aqui são o GROUP BY do mapa (~40 B por
-- linha) e as duas agregações (anúncio-dia → anúncio → grupo, linhas de ~250 B).
-- Com o work_mem global de 3,5 MB da instância, 42 mil linhas ainda escorrem para
-- disco (medido: 27 MB de temporário); com 32 MB locais cabem em memória. É por
-- função, não global: só esta consulta paga, e só enquanto roda.

-- ============================================================================
-- 2. A base
-- ============================================================================

CREATE OR REPLACE FUNCTION public.fetch_manager_performance_base_v130(
...
declare
  ...
  v_data jsonb := '[]'::jsonb;
begin
  select public.fetch_manager_performance_base_v130(
...
'Manager core v2 wrapper: resolve o effective_status de campanha/conjunto a partir de parent_entities (migration 122), enriquece linhas de adset/campanha com budget e ad_count do inventario. Base: fetch_manager_performance_base_v130 (migration 130, read model em passada unica). Rollback: repontar para _base_v116.';
