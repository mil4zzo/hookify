-- Migration 090: métrica "75% View" (video_watched_p75) + métricas por linha no Manager
--
-- 1. Nova coluna ad_metrics.video_watched_p75 (percentual inteiro 0-100 de plays que
--    atingiram 75% do vídeo, mesma semântica de video_watched_p50). Preenchida pelo
--    refresh de packs (Meta insights: video_p75_watched_actions); linhas históricas
--    ficam NULL até o próximo refresh cobrir a data.
--
-- 2. fetch_manager_rankings_core_v2_base_v090: nova implementação base do core RPC.
--    Substitui a cadeia v067 -> v066 -> v060 por uma única função (mesma semântica):
--      - v060 (agregação) + video_watched_p75 e scroll_stop por linha e nas averages
--      - v066 (thumb_storage_path) foldado no CTE rep_ads
--      - v067 (filtro p_campaign_id pós-agregação, pagination resetada) foldado no final
--    A entry fetch_manager_rankings_core_v2 (16 args, resolução de status de pais da
--    migration 089) é recriada apontando para a v090. Wrappers antigos ficam intactos.
--
-- 3. fetch_manager_rankings_series_v2: séries novas para sparklines das colunas novas
--    (video_watched_p75, plays, reach, thruplays). frequency é derivada no cliente
--    (impressions / reach).

alter table public.ad_metrics
  add column if not exists video_watched_p75 integer;

comment on column public.ad_metrics.video_watched_p75 is
  'Percentual inteiro (0-100) de plays que atingiram 75% do vídeo (video_p75_watched_actions / video_play_actions). NULL em linhas anteriores à migration 090 ainda não re-sincronizadas.';

--
-- Core RPC: implementação v090
--

create or replace function public.fetch_manager_rankings_core_v2_base_v090(
  p_user_id uuid,
  p_date_start date,
  p_date_stop date,
...
comment on function public.fetch_manager_rankings_core_v2_base_v090(uuid, date, date, text, uuid[], text[], text, text, text, text, boolean, boolean, integer, integer, text, text) is
  'Manager core v2 base v090: v060 + video_watched_p75/scroll_stop por linha e nas averages, thumb_storage_path (fold do v066) e filtro p_campaign_id pós-agregação (fold do v067).';

grant all on function public.fetch_manager_rankings_core_v2_base_v090(uuid, date, date, text, uuid[], text[], text, text, text, text, boolean, boolean, integer, integer, text, text) to authenticated;
grant all on function public.fetch_manager_rankings_core_v2_base_v090(uuid, date, date, text, uuid[], text[], text, text, text, text, boolean, boolean, integer, integer, text, text) to service_role;

--
-- Entry: recriada apontando para a base v090 (corpo idêntico ao da migration 089,
-- que resolve effective_status de adset/campanha pelo status oficial denormalizado).
--
...
comment on function public.fetch_manager_rankings_core_v2(uuid, date, date, text, uuid[], text[], text, text, text, text, boolean, boolean, integer, integer, text, text) is
  'Manager core v2 wrapper: resolves campaign/adset effective_status preferring the official denormalized parent status (ads.adset_status/campaign_status), falling back to hierarchical pause markers for pre-backfill rows. Base: v090 (video_watched_p75 + scroll_stop por linha).';

--
-- Series RPC: séries novas (video_watched_p75, plays, reach, thruplays)
--
...
create or replace function public.fetch_manager_rankings_series_v2(
  ...
  p_ad_name_contains text default null::text,
  p_action_type text default null::text,
  p_group_keys text[] default null::text[],
...
comment on function public.fetch_manager_rankings_series_v2(uuid, date, date, text, uuid[], text[], text, text, text, text, text[], integer) is
  'Manager series v2 RPC: returns sparkline series for requested group_keys, including clicks, inline_link_clicks, cpc, cplc, retention metrics and (v090) video_watched_p75, plays, thruplays and reach.';
