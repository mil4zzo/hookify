-- 142: "sem audio detectavel" deixa de ser uma resposta que so o dialog de /packs conhece
-- ============================================================================
--
-- PROBLEMA
-- A linha do Manager dizia apenas `has_transcription: true|false`. O "false" ali
-- junta duas coisas de natureza oposta:
--   * video AINDA NAO TRANSCRITO  -> acionavel; basta mandar transcrever;
--   * video SEM AUDIO DETECTAVEL  -> falha permanente; nunca vai ser transcrito.
-- Quem so tinha o booleano prometia ao usuario uma acao que o backend ia recusar:
-- o dialog de export oferecia "Transcrever 12" e o job voltava com "nenhuma
-- transcricao pendente", sem explicar nada.
--
-- A distincao ja existia, mas so em GET /packs/{id}/transcription-status -- uma
-- requisicao POR PACK que le todos os anuncios do pack. Trazer a mesma resposta
-- para dentro da linha nao custa requisicao nem join novo: e a MESMA lateral que
-- ja calculava o booleano, agregando em vez de parar no primeiro acerto.
--
-- O QUE MUDA
-- 1. `fetch_manager_performance_base_v142` = a v140 com a lateral de transcricao
--    devolvendo dois booleanos (`has_transcription`, `no_audio`) em vez de um, e o
--    novo `transcription_no_audio` entrando no payload da linha.
-- 2. `fetch_manager_rankings_core_v2` -- a entrada real que o backend chama --
--    passa a apontar para a v142. A v140 fica no banco sem chamador, como historico.
-- 3. Backfill das linhas de falha antigas que so tinham a frase de "sem audio" no
--    `error_message`, sem a flag `no_voice_detected` (ver o UPDATE no fim).
--
-- CUSTO, MEDIDO NO SHAPE E NAO NO RELOGIO
-- `ad_transcriptions` tem UNIQUE (user_id, ad_name). A lateral antiga lia no maximo
-- uma linha por dono e parava no `limit 1`; a nova le exatamente as mesmas linhas
-- (<=1 por dono; v_owners tem 1, ou 2 em pack compartilhado) pelo mesmo index scan
-- e agrega. Nenhuma tabela nova, nenhuma varredura nova.
--
-- COMPATIBILIDADE
-- Chave NOVA no payload; `has_transcription` continua com o mesmo nome, tipo e
-- significado. Frontend antigo ignora a chave extra (o schema Zod descarta o que
-- nao declara) -- ordem de deploy livre.
-- ============================================================================
...
COMMENT ON FUNCTION public.fetch_manager_performance_base_v142(uuid, date, date, text, uuid[], text[], text, text, text, text, boolean, boolean, integer, integer, text, text, boolean) IS
  'Manager base v142 (migration 142): a v140 + transcription_no_audio na linha. A lateral de ad_transcriptions passa a distinguir "sem audio detectavel" (falha permanente) de "ainda nao transcrito" (acionavel), sem requisicao nem join novo -- mesma UNIQUE (user_id, ad_name), mesmo index scan.';

GRANT EXECUTE ON FUNCTION public.fetch_manager_performance_base_v142(uuid, date, date, text, uuid[], text[], text, text, text, text, boolean, boolean, integer, integer, text, text, boolean) TO anon, authenticated, service_role;


-- ============================================================================
-- 2. Entrada: repontar para a v142
-- ============================================================================
-- Recriada por inteiro porque CREATE OR REPLACE nao troca uma linha isolada do
-- corpo. O UNICO delta em relacao ao que esta em producao e o nome da base chamada.
...
GRANT EXECUTE ON FUNCTION public.fetch_manager_rankings_core_v2(uuid, date, date, text, uuid[], text[], text, text, text, text, boolean, boolean, integer, integer, text, text, boolean) TO anon, authenticated, service_role;


-- ============================================================================
-- 3. Backfill: falhas antigas de "sem audio" sem a flag
-- ============================================================================
-- Os dois caminhos de escrita de falha do worker ja gravam `no_voice_detected`
-- quando a mensagem casa (transcription_worker.py, via
-- supabase_repo.is_no_audio_error_message). Restam as linhas gravadas ANTES dessa
-- normalizacao, que so tem a frase no `error_message`. Sem este backfill elas
-- apareceriam para sempre como "ainda nao transcrito" e o botao de transcrever as
-- ofereceria para um job que sempre as recusa.
--
-- Este UPDATE existe para nao precisar repetir a lista de frases dentro do SQL da
-- RPC: depois dele, a unica fonte de verdade no banco e a flag booleana.
-- Conferido antes de escrever (mesmo WHERE, em SELECT count): 2 linhas em
-- producao em 2026-09-05. E idempotente -- rodar de novo nao casa mais nada.

update public.ad_transcriptions
set metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object('no_voice_detected', true)
where status = 'failed'
  and coalesce(metadata ->> 'no_voice_detected', '') <> 'true'
  and (
    lower(coalesce(metadata ->> 'error_message', '')) like '%no spoken audio%'
    or lower(coalesce(metadata ->> 'error_message', '')) like '%no audio stream%'
  );
