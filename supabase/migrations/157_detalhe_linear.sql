...
REVOKE ALL ON FUNCTION public.fetch_entity_performance_v157(uuid, date, date, text, text, uuid[], text, boolean, integer, boolean) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.fetch_entity_performance_v157(uuid, date, date, text, text, uuid[], text, boolean, integer, boolean) TO authenticated, service_role;

COMMENT ON FUNCTION public.fetch_entity_performance_v157(uuid, date, date, text, text, uuid[], text, boolean, integer, boolean) IS
'Detalhe de uma entidade (157): a v155 com a montagem final linear no número de anúncios (partes empilhadas e agregadas por grupo, sem junção entre etapas). Saída idêntica à v155.';
