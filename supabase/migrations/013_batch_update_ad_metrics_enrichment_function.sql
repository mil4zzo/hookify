-- Migration: Create batch update function for ad_metrics enrichment
-- Permite atualizar milhares de registros em uma única transação via RPC
-- Reduz de N requisições HTTP para apenas 1, melhorando drasticamente performance
...
COMMENT ON FUNCTION public.batch_update_ad_metrics_enrichment IS 
  'Atualiza múltiplos registros de ad_metrics em uma única transação. Recebe array de updates com ids, leadscore_values e cpr_max. Muito mais eficiente que múltiplas requisições HTTP individuais. Reduz de N requisições para apenas 1.';
