-- Migration 042
-- Manager v2 final RPC pipeline:
-- - core: aggregated rows + header + pagination
-- - series: sparkline data for requested group_keys (window)
-- - retention: weighted retention curve by group_key
...
begin
  ...
  if v_selected_key like 'conversion:%' then
    ...
    -- Legacy compatibility: raw action name means conversion source.
    v_action_source := 'conversion';
...
'Manager core v2 RPC: aggregated rows + header + pagination, dedup by (user_id, ad_id, date), selected action metrics only.';
...
'Manager series v2 RPC: returns sparkline series for requested group_keys, fixed window, selected action only, includes missing keys with empty/null series.';

create or replace function public.fetch_manager_rankings_retention_v2(
  p_user_id uuid,
  p_date_start date,
  p_date_stop date,
  p_group_by text default 'ad_name',
...
grant execute on function public.fetch_manager_rankings_retention_v2(
  uuid, date, date, text, uuid[], text[], text, text, text, text
) to authenticated;

comment on function public.fetch_manager_rankings_retention_v2(
  uuid, date, date, text, uuid[], text[], text, text, text, text
) is
'Manager retention v2 RPC: returns weighted retention curve for one group_key, loaded on demand.';
