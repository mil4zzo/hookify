-- Migration 117: tags por linha na RPC do Manager (base v116)
--
-- O QUE MUDA
--   1. Cada linha passa a carregar `tags`: [{id,name,color}] do ATOR, nos niveis
--      de criativo (ad_name) e anuncio (ad_id). Em conjunto/campanha vem [].
--   2. A thumbnail do CDN da Meta para de trafegar quando ha thumb_storage_path.
--
-- POR QUE O JOIN E POR p_user_id, NAO PELO DONO DAS LINHAS
--   Desde a v104 a RPC e dirigida por v_owners: num pack compartilhado as linhas
--   de ad_metrics sao do DONO, enquanto p_user_id e o ATOR que pergunta. Tag e
--   pessoal — juntar pelo dono faria o convidado enxergar o vocabulario interno
--   de quem compartilhou ('cliente-acme', 'testar-antes-de-cancelar').
--
-- POR QUE O ENRIQUECIMENTO ENTRA DEPOIS DE rep, NAO EM ad_metrics
--   ad_metrics tem 250k linhas; os grupos sao centenas. Juntar ad_tags no caminho
--   de varredura diaria pagaria o custo por linha-dia. Ancorado em rep (um registro
--   por group_key) o join e contra um punhado de linhas e usa ad_tags_user_name_idx.
--
-- POR QUE A PODA DA THUMBNAIL E RESTRITA A ad_name/ad_id
--   Medido: a URL do Meta e 21% do payload que sai do banco (~194 kB por chamada
--   de 371 criativos) e o backend a DESCARTA — _hydrate_storage_thumbnails_for_
--   rankings_rows reescreve tudo a partir de thumb_storage_path, e o fallback para
--   o CDN da Meta esta desligado por decisao anterior.
--
--   MAS get_rankings_adset_children (analytics.py) consome a RPC SEM hidratar.
--   Esse caminho usa group_by='adset_id' — por isso a poda exclui esse nivel.
--
-- POR QUE UMA BASE NOVA (v116) EM VEZ DE REPLACE NA v115
--   Mesma disciplina das v093/v104/v105/v115: a base antiga fica no banco e o
--   wrapper troca de alvo numa linha. Rollback e reapontar o wrapper, sem
--   recriar 700 linhas de SQL sob pressao.
...
-- Wrapper passa a apontar para a v116.
