-- Migration 139: tag e do SILO DO PACK, nao de quem olha
--
-- O QUE MUDA
--   1. ad_tags.created_by: quem marcou. A posse passa a ser do silo, entao sem
--      esta coluna "quem marcou isso?" deixa de ter resposta num pack compartilhado.
--   2. Base v139: o join de tags le por any(v_owners) em vez de p_user_id.
--
-- POR QUE
--   A tag nasceu pessoal (v116) por receio de vazar vocabulario interno. Na pratica
--   o fluxo e o oposto: quem compartilha um pack quer que os editores classifiquem
--   junto, e dois vocabularios invisiveis um para o outro so produzem duplicata.
--
--   E o proprio codigo ja tinha respondido isso. Dentro da MESMA funcao, as duas
--   laterais vizinhas ao join de tags leem por silo do pack:
--
--     media_type        -> a.user_id = any(v_owners)   (public.ads)
--     has_transcription -> t.user_id = any(v_owners)   (public.ad_transcriptions)
--     tags              -> atg.user_id = p_user_id     <- unica fora do padrao
--
--   Transcricao de pack compartilhado JA e vista pelo convidado. Tag pessoal era
--   inconsistencia, nao decisao de arquitetura.
--
-- CUSTO (medido em 2026-09-03, pack real EI.30 - Captacao CA05, quente)
--   v138: 314,7 ms / 300,5 ms
--   v139: 299,4 ms / 300,5 ms
--   Identico. A lateral roda DEPOIS do agrupamento, sobre centenas de linhas,
--   contra uma tabela pequena, pelo indice ad_tags_user_name_idx (user_id, ad_name),
--   e usa a mesma forma any(v_owners) das vizinhas. Nao encosta no read model
--   (ad_performance_daily) nem na agregacao do rollup 128-138.
--
-- MULTI-DONO
--   Leitura e multi-silo por construcao: any(v_owners) cobre todos os donos dos
--   packs selecionados. Duas tags de mesmo NOME em donos diferentes aparecem as
--   duas — sao tags distintas, e fundi-las esconderia de quem e cada uma.
--   A ESCRITA e que exige um dono so; isso e gateado no backend (routes/tags.py).

BEGIN;

-- ---------------------------------------------------------------------------
-- created_by: autoria sobrevive a mudanca de posse
-- ---------------------------------------------------------------------------
ALTER TABLE public.ad_tags ADD COLUMN IF NOT EXISTS created_by uuid;

-- Backfill: ate aqui a tag era do proprio ator, entao dono == autor.
UPDATE public.ad_tags SET created_by = user_id WHERE created_by IS NULL;

COMMENT ON COLUMN public.ad_tags.created_by IS
  'Quem aplicou a marcacao. user_id e o SILO (dono do pack); created_by e o ATOR. Num pack compartilhado sao pessoas diferentes, e este e o unico registro dessa autoria.';

CREATE OR REPLACE FUNCTION public.fetch_manager_performance_base_v139(p_user_id uuid, p_date_start date, p_date_stop date, p_group_by text DEFAULT 'ad_name'::text, p_pack_ids uuid[] DEFAULT NULL::uuid[], p_account_ids text[] DEFAULT NULL::text[], p_campaign_name_contains text DEFAULT NULL::text, p_adset_name_contains text DEFAULT NULL::text, p_ad_name_contains text DEFAULT NULL::text, p_action_type text DEFAULT NULL::text, p_include_leadscore boolean DEFAULT true, p_include_available_conversion_types boolean DEFAULT true, p_limit integer DEFAULT 500, p_offset integer DEFAULT 0, p_order_by text DEFAULT 'spend'::text, p_campaign_id text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- Entry passa a apontar para a v139.
...
declare
  ...
  v_data jsonb := '[]'::jsonb;
begin
  select public.fetch_manager_performance_base_v139(
