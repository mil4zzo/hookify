-- =====================================================================
-- 137 - Procedencia dos pais SEMPRE na linha (o interruptor da 136 sai)
-- =====================================================================
--
-- O QUE MUDA EM RELACAO A 136
--   A 136 introduziu `campaign_ids`/`adset_ids` na linha e o dicionario `names` na
--   raiz, atras de um opt-in (`p_include_parent_ids`, default false), porque a
--   medicao de 500 linhas sugeria +67% de payload.
--
--   Remedida no tamanho REAL do Manager (que pede ate 10.000 linhas), a conta e bem
--   menor - o gzip aproveita a repeticao dos ids em muito mais linhas:
--
--     Manager | 36 packs | 30 dias  |   407 criativos:  82 kB -> 111 kB  (+29 kB)
--     Manager | 36 packs | 13 meses | 1.739 criativos: 721 kB -> 918 kB  (+197 kB)
--
--   Diante disso o idealizador decidiu ligar sempre: padroniza o contrato (toda
--   resposta tem os mesmos campos), e as telas que hoje nao filtram por campanha
--   provavelmente vao filtrar. O interruptor sai inteiro - do SQL, do backend e das
--   quatro telas - em vez de virar um `default true` que ninguem mais desliga.
--
-- O QUE NAO MUDA
--   Todo o resto e a v136 com `p_include_parent_ids := true` fixo. A equivalencia e
--   verificada por `jsonb =` contra a v136 ligada, o que herda o diferencial completo
--   que a 136 ja passou (484 cenarios contra a v132).
--
-- O PROBLEMA QUE ISSO RESOLVE (contexto, da 136)
--   Uma linha agrupada por criativo colapsa dezenas de anuncios espalhados por varias
--   campanhas, e so carregava a campanha do REPRESENTANTE. Filtrar por ela acertava a
--   maioria e MENTIA no resto, em silencio: 390 de 1.739 criativos (22%) rodam em mais
--   de uma campanha. "Me mostre os criativos da campanha X" escondia todo criativo cuja
--   maior entrega estivesse em outra campanha - sem erro, a linha so nao aparecia.
--
-- A ENTRADA VOLTA A 16 PARAMETROS
--   `fetch_manager_rankings_core_v2` perdeu o 17o. `create or replace` nao remove
--   parametro: deixaria as DUAS sobrecargas vivas e o PostgREST nao saberia qual
--   chamar (PGRST203). O `drop` da versao de 17 argumentos vem antes, na mesma
--   transacao - entre o drop e o create a funcao nao existe, e uma requisicao viva
--   nesse instante levaria "function does not exist".
--
...
-- A entrada volta aos 16 parametros e aponta para a v137.
drop function if exists public.fetch_manager_rankings_core_v2(
  uuid, date, date, text, uuid[], text[], text, text, text, text, boolean, boolean, integer, integer, text, text, boolean
);
...
begin
  ...
  if v_sobrecargas is distinct from 1 then
    raise exception '137: a entrada ficou com % sobrecargas - o PostgREST nao saberia qual chamar', v_sobrecargas;
  end if;
  ...
  if v_alvo is distinct from 'fetch_manager_performance_base_v137' then
    raise exception '137: a entrada continua apontando para %', coalesce(v_alvo, '<nenhuma>');
  end if;
  if v_args like '%p_include_parent_ids%' then
    raise exception '137: o parametro p_include_parent_ids sobreviveu na entrada';
  end if;
  raise notice '137 OK: fetch_manager_rankings_core_v2 (1 sobrecarga, sem opt-in) -> fetch_manager_performance_base_v137';
end $$;
