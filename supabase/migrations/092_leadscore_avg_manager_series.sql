-- Migration 092: série diária "leadscore_avg" para a coluna opcional "Leadscore médio" do Manager.
--
-- A média de leadscore por linha (modo "Média" do toggle) já é 100% derivável no cliente a
-- partir de `leadscore_values` (mesmo padrão de mqls/cpmql — ver computeMqlMetricsFromLeadscore).
-- Não precisa de mudança na core RPC (fetch_manager_rankings_core_v2_base_v090) nem em
-- averages_payload: assim como mqls/cpmql, a média (default e filtrada) é sempre calculada
-- no cliente via computeManagerAverages, nunca vem do servidor.
--
-- O que falta apenas no servidor é a série DIÁRIA (modo "Tendências"/sparkline), que precisa
-- de agregação por dia — replicado aqui em fetch_manager_rankings_series_v2 no mesmo padrão
-- de mql_count/cpmql (soma de leadscore_values do dia / contagem de leadscore_values do dia).
...
comment on function public.fetch_manager_rankings_series_v2(uuid, date, date, text, uuid[], text[], text, text, text, text, text[], integer) is
  'Manager series v2 RPC: returns sparkline series for requested group_keys, including clicks, inline_link_clicks, cpc, cplc, retention metrics, video_watched_p75/plays/thruplays/reach (v090) and leadscore_avg (v092, weighted by lead count, same daily-aggregation pattern as mqls/cpmql).';
