-- Migration 093: série diária "mql_rate" para a coluna opcional "% de MQLs" do Manager.
--
-- mql_rate = taxa de qualificação = MQLs / TOTAL de leads (não MQLs / não-MQLs). Escala 0-1
-- (formatKind ratioPercent no frontend), null quando o dia não teve lead nenhum.
--
-- Assim como leadscore_avg (migration 092) e mqls/cpmql, o valor POR LINHA e as médias
-- (default e filtrada) são 100% derivados no cliente a partir de `leadscore_values` —
-- a core RPC (fetch_manager_rankings_core_v2_base_v090) e averages_payload NÃO mudam.
--
-- O único trabalho de servidor é a série DIÁRIA (modo "Tendências"/sparkline), e aqui ela sai
-- de graça: o CTE `daily` já acumula `mql_count` (desde a versão original) e `leadscore_count`
-- (desde a 092). mql_rate é só a divisão dos dois — nenhum acumulador novo.
...
comment on function public.fetch_manager_rankings_series_v2(uuid, date, date, text, uuid[], text[], text, text, text, text, text[], integer) is
  'Manager series v2 RPC: returns sparkline series for requested group_keys, including clicks, inline_link_clicks, cpc, cplc, retention metrics, video_watched_p75/plays/thruplays/reach (v090), leadscore_avg (v092) and mql_rate (v093 = mql_count / leadscore_count, qualification rate on a 0-1 scale).';
